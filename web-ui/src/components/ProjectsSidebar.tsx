...
function syncLayout(layout: ProjectsLayout, projects: RuntimeProject[]): ProjectsLayout {
  ...
  const topLevel = layout.topLevel.filter(
    (i) => i.type === "folder" || known.has(i.workspaceId),
  );
  const folders = Object.fromEntries(
    Object.entries(layout.folders).map(([id, f]) => [
      id, { ...f, projectIds: f.projectIds.filter((id) => known.has(id)) },
    ]),
  );
  const inLayout = new Set<string>();
  topLevel.forEach((i) => { if (i.type === "project") inLayout.add(i.workspaceId); });
...
export function ProjectsSidebar({ projects, activeWorkspaceId, onSwitch }: Props) {
  ...
  const update = (next: ProjectsLayout) => { setLayout(next); persist(next); };
                ...
                return (
                  <Draggable key={`fh:${item.folderId}`} draggableId={`fh:${item.folderId}`} index={index}>
                    {(dp, snap) => (
                      ...
                      >
                        <span {...dp.dragHandleProps} className="shrink-0 text-gray-700 hover:text-gray-400 cursor-grab active:cursor-grabbing">
                        ...
                        {editingId === item.folderId ? (
                          ...
                          <span className="flex-1 min-w-0 truncate font-medium" onDoubleClick={() => startRename(item.folderId)}>
                            {folder.name}
                          </span>
                        )}
                        <span className="shrink-0 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => startRename(item.folderId)} className="p-0.5 hover:text-gray-200" title="Rename"><Pencil size={10} /></button>
                          <button onClick={() => deleteFolder(item.folderId)} className="p-0.5 hover:text-red-400" title="Delete"><Trash2 size={10} /></button>
                        </span>
                      </div>
                    )}
                  </Draggable>
                ...
                <Draggable key={`p:${item.workspaceId}`} draggableId={`p:${item.workspaceId}`} index={index}>
                  {(dp, snap) => (
                    ...
                    >
                      ...
                      <button onClick={() => onSwitch(item.workspaceId)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
                        <span className={`size-1.5 rounded-full shrink-0 ${isActive ? "bg-blue-400" : "bg-gray-700"}`} />
