	...
	return (
		<div className="flex flex-col w-60 shrink-0">
			<div className="flex items-center gap-1.5 px-2.5 py-2 shrink-0">
				<span className={classNames("size-2 rounded-full shrink-0", COLUMN_DOT_COLORS[column.id] ?? "bg-[#5f6672]")} />
				<span className="text-xs font-medium text-[#8a8f98]">{column.title}</span>
				<span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#161616] text-[#8a8f98] font-medium">
					{cards.length}
				</span>
				<div className="flex-1" />
				<button onClick={onAddCard} className="text-[#5f6672] hover:text-[#ededed] transition-colors" title="Add task">
					<Plus size={13} />
				</button>
			</div>
				...
				{(provided, snapshot) => (
					<div
						...
						className={classNames(
							...
							snapshot.isDraggingOver ? "bg-[#111111]/40" : "",
						)}
					>
