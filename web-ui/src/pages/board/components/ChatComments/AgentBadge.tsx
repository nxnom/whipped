...
export function AgentBadge({ comment }: { comment: RuntimeReviewComment }) {
	...
	if (actor.type === "external") {
		...
		return (
			<span className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-medium text-whip-muted bg-whip-border/50">
				{label}
			</span>
		);
	}
	...
	const className = MODEL_STYLE[actor.id] ?? "text-whip-muted bg-whip-border/50";
	return (
		<span
		...
		>
			...
			{status === "skipped" && <span className="text-whip-muted">—</span>}
		</span>
	);
}
