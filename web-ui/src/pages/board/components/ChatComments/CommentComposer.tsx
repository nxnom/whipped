...
export function CommentComposer({ card, workspaceId, onRefresh }: CommentComposerProps) {
	...
	return (
		<>
			<div className="shrink-0 border-t border-whip-border-soft p-3">
				<input
					ref={fileInputRef}
					type="file"
					accept="*/*"
				...
				<div className="rounded-lg border border-whip-border bg-whip-bg focus-within:border-whip-border-hover transition-colors">
								...
								<div key={att.n} className="relative group">
									...
									{att.dataUrl ? (
										<img
											src={att.dataUrl}
											alt={att.name}
											className="h-16 w-16 object-cover rounded border border-whip-border"
											title={att.name}
										/>
									) : (
										<div
											className="h-16 w-16 flex flex-col items-center justify-center gap-1 rounded border border-whip-border bg-whip-panel-2 px-1"
											title={att.name}
										>
											<Paperclip size={16} className="shrink-0 text-whip-muted" />
											<span className="text-[10px] text-whip-muted w-full text-center truncate">{att.name}</span>
										</div>
									)}
									<button
										onClick={() => removeAttachment(att.n)}
										className="absolute -top-1 -right-1 size-4 rounded-full bg-whip-panel-2 border border-whip-border-hover flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
									>
										<X size={10} className="text-whip-text" />
									</button>
								</div>
							))}
						</div>
					...
					{visualComment && visualComment.elements.length > 0 && (
						<div className="mx-3 mt-2 flex flex-col gap-1.5 rounded-lg border border-whip-border bg-whip-panel p-2">
							<div className="flex items-center gap-2">
								<Crosshair size={12} className="text-[#8b5cf6]" />
								<span className="text-[11px] font-medium text-whip-muted">
								...
								<button
									...
									className="text-[11px] text-whip-faint hover:text-[#ff3b4d] transition-colors"
								>
									Clear
								</button>
							</div>
							<div className="flex flex-wrap gap-1.5">
								{visualComment.elements.map((el, i) => {
									...
									return (
										<span
											key={i}
											className="inline-flex items-center gap-1 rounded border border-whip-border bg-whip-panel-2 px-1.5 py-0.5 text-[10px] text-whip-muted"
											title={el.elementSelector}
										>
											<span className="text-[#c4b5fd]">#{i + 1}</span>
											<span className="truncate max-w-[180px]">🧩 {label}</span>
											{src && <span className="text-whip-faint font-mono">📄 {src}</span>}
										</span>
									);
								})}
							</div>
					...
					<TokenTextarea
						...
						rows={5}
						metricsClassName="text-sm text-whip-text px-3 pt-3 pb-1 leading-normal placeholder-whip-faint"
					/>
					<div className="flex items-center justify-between px-3 pb-2">
						<div className="flex items-center gap-2">
							...
							<span className="text-[10px] text-whip-faint">↵ Send · ⇧↵ Newline</span>
						</div>
