...
export function ReopenPickerDialog({
	...
	return (
		<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70" onClick={onClose}>
			<div
				className="flex flex-col gap-4 bg-whip-surface rounded-xl border border-whip-border w-[420px] max-w-[90vw] p-5 shadow-[0_8px_40px_4px_rgba(0,0,0,0.38)]"
				onClick={(e) => e.stopPropagation()}
			>
				<div className="flex items-center gap-3">
					<span className="text-[14px] font-semibold text-whip-text flex-1">Reopen — tier for the rework</span>
					<button onClick={onClose} className="hover:opacity-70 transition-opacity">
						<X size={18} className="text-whip-faint" />
					</button>
				</div>

				<p className="text-[12px] text-whip-muted leading-relaxed">
				...
				<div className="flex flex-col gap-1.5">
					<span className="text-[11px] font-medium text-whip-faint tracking-[0.3px]">Tier</span>
