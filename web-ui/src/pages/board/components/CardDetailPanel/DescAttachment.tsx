...
export function DescAttachment({ path, name, mimeType }: { path: string; name: string; mimeType?: string }) {
	...
	return (
		<a
			...
			rel="noreferrer"
			title={name}
			className="flex items-center gap-1.5 px-2 py-1 rounded border border-whip-border bg-whip-panel text-xs text-whip-text hover:text-whip-text hover:border-whip-border-hover transition-colors max-w-[160px] truncate"
		>
