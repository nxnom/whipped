		...
		<>
			...
			<FormProvider key={mode} {...(activeMethods as unknown as UseFormReturn<FieldValues>)}>
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					{/* Dialog */}
					<div className="relative flex h-[850px] max-h-[calc(100vh-80px)] w-[1400px] max-w-[calc(100vw-80px)] rounded-xl bg-whip-surface border border-whip-border shadow-[0_8px_40px_4px_#00000060] overflow-hidden">
						{/* ── Left panel ── */}
						<div className="flex flex-col flex-1 overflow-hidden">
							{/* Header */}
							<div className="flex items-center gap-3 px-6 py-3.5 border-b border-whip-border shrink-0">
								<span className="text-[15px] font-semibold text-whip-text">{isTask ? "New Task" : "New Story"}</span>
							...
							<div className="flex flex-col flex-1 min-h-0 px-6 py-4 gap-2">
								...
								{!isTask && (
									<div className="flex items-center gap-1.5 shrink-0">
										<span className="text-[11px] font-medium text-whip-faint">Story Objective</span>
										<div className="flex-1" />
										<span className="text-[10px] text-whip-faint">The orchestrator will break this into subtasks</span>
									</div>
								)}
		...
		<FormProvider {...methods}>
			<div className="fixed inset-0 z-50 flex items-center justify-center">
				...
				<div className="relative flex h-[850px] max-h-[calc(100vh-80px)] w-[1400px] max-w-[calc(100vw-80px)] rounded-xl bg-whip-surface border border-whip-border shadow-[0_8px_40px_4px_#00000060] overflow-hidden">
					...
					<div className="flex flex-col flex-1 overflow-hidden">
						<div className="flex items-center gap-3 px-6 py-3.5 border-b border-whip-border shrink-0">
							<span className="text-[15px] font-semibold text-whip-text">{dialogTitle}</span>
							<div className="flex-1" />
					...
					<div className="w-80 shrink-0 bg-whip-bg border-l border-whip-border flex flex-col overflow-hidden">
						...
						<div className="flex items-center gap-2.5 px-[18px] py-3.5 border-t border-whip-border shrink-0">
							...
							<button
								onClick={onClose}
								className="px-4 py-2 rounded-md text-xs font-medium text-whip-muted hover:text-whip-text transition-colors"
							>
								Cancel
							</button>
							<button
								...
								className="flex items-center gap-1.5 px-5 py-2 rounded-md text-xs font-semibold text-whip-accent-text bg-whip-accent disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
							>
			...
			<div className="fixed inset-0 z-[60] flex items-center justify-center">
				...
				<div className="relative flex h-[850px] max-h-[calc(100vh-80px)] w-[1400px] max-w-[calc(100vw-80px)] rounded-xl bg-whip-surface border border-whip-border shadow-[0_8px_40px_4px_#00000060] overflow-hidden">
					...
					<div className="w-80 shrink-0 bg-whip-bg border-l border-whip-border flex flex-col overflow-hidden">
						...
						<div className="flex-1 min-h-0 overflow-y-auto px-[18px] py-4 flex flex-col gap-5">
							<div className="flex flex-col gap-2">
								<span className="text-[11px] font-medium text-whip-faint">Workflow</span>
								<RHFSelect
									name="workflowId"
									onChange={onWorkflowChange}
									prefix={<WorkflowIcon size={14} className="text-whip-muted" />}
							...
							<div className="flex flex-col gap-2">
								<span className="text-[11px] font-medium text-whip-faint">Dependencies</span>
										...
										return (
											<SelectOption key={draft.tempId} value={draft.tempId} label={draftDisplay} hideCheckIcon>
												<div className="flex items-center justify-between w-full gap-2 min-w-0">
													...
													<span className="text-[10px] px-1.5 py-0.5 rounded shrink-0 font-medium text-whip-text bg-whip-text/10">
														this story
													</span>
												</div>
											</SelectOption>
