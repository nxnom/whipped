...
export function defaultPromptPath(
  repoPath: string,
  workflowName: string,
  slotName: string
): string {
...
function PromptLinkContent({
  ...
  const [conflict, setConflict] = useState<{ fileContent: string } | null>(
    null
  );
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-semibold text-[#f0f0f5]">
            Link prompt to a file
          </h3>
        </div>
        <p className="text-[13px] text-[#8888a0] leading-relaxed">
          The agent reads this file at runtime. Edits auto-save back to it, and
          changes you make in your own editor are picked up too.
        </p>
        <div className="flex flex-col gap-1.5">
          <span className="text-[12px] font-medium text-[#c0c0d0]">
            File path
          </span>
