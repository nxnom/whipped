import type { PlanBlock } from "../core/api-contract.js";

// Renders plan blocks into plain text for a system prompt. The plan panel is
// push-only (agent -> human, via whipped_show_plan) — there's no tool for the
// agent to read it back, so this is the only way a resumed session's agent
// ever learns what a saved plan actually contains.
export function serializePlanBlocksForPrompt(blocks: PlanBlock[]): string {
	return blocks
		.map((block) => {
