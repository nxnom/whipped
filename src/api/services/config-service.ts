...
function stripAuthFields(config: RuntimeGlobalConfig): RuntimeGlobalConfig {
	const { authPasswordHash: _hash, authSessionSecret: _secret, authMachineToken: _token, ...rest } = config;
...
export const saveGlobalConfig = async (patch: Partial<RuntimeGlobalConfig>) => {
	const { authPasswordHash: _hash, authSessionSecret: _secret, authMachineToken: _token, ...safe } = patch;
